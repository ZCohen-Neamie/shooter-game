...
for row_index, row in enumerate(level_map):
    for col_index, cell in enumerate(row):
        if cell == "1":
            ...
            if is_border:
                block = TerrainBlock(block_x, block_y, block_size, color=(50,50,50),destructible=False)
            else:
...
while running:
    ...
    for event in pygame.event.get(): # take all events generated in a frame during game play 
